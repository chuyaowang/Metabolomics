...
```{r loadData}
datadir <- "./data/20230208"
file <- list.files(path = datadir, pattern = "^.*\\.mzXML$",
                   all.files = TRUE, full.names = TRUE,
                   recursive = FALSE, ignore.case = FALSE,
                   include.dirs = FALSE, no.. = TRUE)
pd <- data.frame(file = basename(file),
                 sample = c("Sample1", "Sample2","Sample3",'Sample4'),
                 group = c("6h","28h","49h","72h"))
...
data_cent %>%
  filterFile(1) %>%
  filterMz(mz=get_ppm_range(179.056,5)) %>%
  filterRt(rt=c(380,440)) %>%
  plot(type="XIC")
```
...
data_cent %>%
  filterFile(1) %>%
  filterMz(mz=get_ppm_range(173.1044,5)) %>%
  filterRt(rt=c(800,900)) %>%
  chromatogram %>%
  plot
```
...
params <- CentWaveParam(ppm = 5,
                        peakwidth = c(3.786432,143),
                        snthresh = 20,
                        prefilter = c(3,70779.88),
                        mzCenterFun = "wMeanApex3",
                        integrate = 1,
                        mzdiff = -0.001, # if negative allow overlap, if positive and <0.001, count as one peak, if >0.001, count as two peaks
                        noise = 1e5
...
```{r, warning=FALSE}
chr_glu <- chromatogram(data_cent, mz=get_ppm_range(185.0762,5),rt=c(380,450))
...
```{r}
compounds <- read.xlsx("data/metabolites.xlsx",sheet=1,startRow = 1)
...
count <- mutate(count, ion_formula_neg = paste("C",C,"H",H-1,"N",N,"O",O,"P",P,"S",S,sep="")) # change to H+1 for positive mode
for (i in seq_along(count$ion_formula_neg)) {
  vec <- str_extract_all(count$ion_formula_neg[i],"\\D?\\d+") %>% unlist 
  vec[str_detect(vec,"^\\D?0$")] <- ""
  vec[str_detect(vec,"^\\D?1$")] <- str_remove(vec[str_detect(vec,"^\\D?1$")],"1")
  count$ion_formula_neg[i] <- str_flatten(vec)
}
...
for (spl in unique(peaklist$sample)) {
  for (cpd_idx in seq_along(compounds$Name)) {
    ...
    areas <- c() # stores area for each mz value
    for (mz_idx in seq_along(mzs)) {
      temp <- peaklist %>%
        filter(sample == spl) %>% 
        ...
        unlist
      temp <- c(temp,0)
      areas[mz_idx] <- max(temp,na.rm = TRUE)
    }
    
    if (sum(areas)==0) {
      mdv[cpd_idx,spl] <- rep(0,times=compounds$C[cpd_idx]) %>% as.character() %>% str_flatten(.,collapse = " ")
    } else {
      mdv[cpd_idx,spl] <- (areas/sum(areas)) %>% round(.,digits=4) %>% as.character() %>% str_flatten(.,collapse = " ") 
      
      isocor.sample <- rep(paste("Sample_",spl),times=length(areas))
      isocor.metabolite <- rep(abbs[cpd_idx],times=length(areas))
      isocor.derivative <- rep(NA,times=length(areas))
      isocor.isotopologue <- (length(areas)-1):0
      isocor.area <- areas
...
colnames(mdv) <-  fileNames(xdata_fill) %>% basename
...
```{r}
metdat <- compounds %>% 
  select(Abbreviation, ion_formula_neg) %>% 
...
```{r}
write_csv(compounds,"output/mdv1.csv",na="")
write_tsv(isocor,"output/isocordata.tsv",na="")
